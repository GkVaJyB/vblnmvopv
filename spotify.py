...
if 'BOT_TOKEN' in os.environ:
    ...
    token = '1499434144:AAHMVlE7yqjuOc1gvenOW00ZcU74a7whc8Y'
...
def DOWNLOADMP3(link,chat_id):
    ...
    YTSEARCH = str(song + " " + artist + " " + '- "auto-generated by youtube."')
    ...
    aud.tag.save()
    CAPTION = f'┌💾 Info Track\n├🎤 By: *{artist}*\n├🎵 Song name: *{song}*\n├💽Album: *{album}*\n└📆 Released on: *{realese_date}*'
    CAPTIONIG = f'┌💾 Info Track\n├🎤 By: *{artist}*\n├🎵 Song name: *{song}*\n├💽Album: *{album}*\n└📆 Released on: *{realese_date}*\n\n`{songid}`\n———————————————\n-Tag:#*{artist}* #*{realese_date}*'
    bot.sendAudio(chat_id, open(f'song//{trackname}.mp3', 'rb'), title=trackname, caption=CAPTION, parse_mode='Markdown')

    bot.sendPhoto(-1001234144318, open(f'songpicts//{trackname}.png', 'rb'), caption=CAPTIONIG, parse_mode='Markdown', disable_notification=None, reply_to_message_id=None, reply_markup=None)

    bot.sendAudio(-1001234144318, open(f'song//{trackname}.mp3', 'rb'), title=trackname, caption=CAPTION, parse_mode='Markdown')
  
    print('[Telegram]Song sent!')

    os.remove(f'song//{trackname}.mp3')
    os.remove(f'songpicts//{trackname}.png')
